...
what();
    function what(){
        var m3u8url = window.location;
		document.getElementById("url").innerHTML = '<a href='+m3u8url+' target="_blank">'+m3u8url+'</a>';
		var vid = document.getElementById("video");
		vid.volume = 0.6;
		var rooturl = m3u8url.toString().substring(64);
		$('#url0').val(rooturl);
};

$(".gourl").on("click", function(e){
	var bla = $('#url0').val();
    e.preventDefault();
    $('#form-control').attr('action', "player.html#"+bla).submit();
});
